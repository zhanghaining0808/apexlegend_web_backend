...
from apex_py.models.user import User, UserPublic, UserUpdate, UserUpdateReq
...
def update_user(user_id: int, user_update_req: UserUpdateReq, session: SessionDep):
    ...
    if not need_update_user:
        raise HTTPException(status_code=404, detail="用户新数据未找到！")
    if not find_user:
        raise HTTPException(status_code=404, detail="用户旧数据未找到！")
